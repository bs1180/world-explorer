...
describe("<Search />", () => {
  test("fetchs results via hook", () => {
    ...
    const input = getByLabelText(/Country search/);
  ...
  test("results get rendered", () => {
    (useSearchCountries as jest.Mock).mockReturnValue([
      { name: "Genovia", code: "GEN" },
      { name: "Moldavia", code: "MOV" },
    ]);
    ...
    userEvent.type(getByLabelText(/Country search/), "Genovia");
